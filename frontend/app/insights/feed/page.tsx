...
import FeedHeader, {
  type FeedFilter,
} from "@/app/components/Feed-comp/FeedHeader";
import FeedList, {
  type FeedActivityItem,
} from "@/app/components/Feed-comp/FeedList";
...
const fallbackItems: FeedActivityItem[] = [
  {
    ...
    title:
      "Production Environment: v2.4.0-rc1",
    body:
      "Successfully deployed 14 services and updated the global edge configuration.",
    time: "2 hours ago",
    group: "Today",
    meta: "Deployment",
    image:
  ...
  },
  {
    ...
    body:
      "The insights heatmap needs more padding around the legend on compact displays.",
    time: "Yesterday",
    group: "Yesterday",
    meta: "Discussion",
    image:
      "https://i.pravatar.cc/96?img=22",
    progress: null,
  },
];
    ...
    async function loadFeed() {
      ...
      try {
        ...
        if (
          ...
          setStatus(
            "Showing local activity because the backend feed is unavailable."
          );
        }
      } finally {
        setIsLoading(false);
  ...
  const createInsight =
    async () => {
      ...
      try {
        ...
        setStatus(
          "Insight added to the backend feed."
        );
      } catch {
        ...
        setStatus(
          "Backend was unavailable, so this insight was added locally."
        );
      }
    };
